...
const resources: Record<Language, LocalizationResources> = {
	ja: {
		...
		process_empty:
			"プロセスが空です - 言語サーバーのプロセスが見つかりませんでした",
		...
		requirement_powershell_permission:
			"システムに PowerShell コマンドを実行する権限があること",
		requirement_wmic_permission:
			"システムに wmic/PowerShell および netstat コマンドを実行する権限があること",

		tooltip_details: "クリックして Antigravity Quota の詳細を表示",
		menu_placeholder:
			"ステータスバーでの表示/非表示を切り替えるモデルを選択してください",
	...
	en: {
		...
		requirement_powershell_permission:
			"The system has permission to run PowerShell commands",
		requirement_wmic_permission:
			"The system has permission to run wmic/PowerShell and netstat commands",
